...
function getPinchProps(touches) {
	var angle = Math.atan((touches[1].pageY - touches[0].pageY) / (touches[1].pageX - touches[0].pageX)) * 180 / Math.PI;
	if(this._lastPinch && Math.abs(this._lastPinch.angle-angle) > 90) {
		angle += this._lastPinch.angle-angle > 0 ? 180 : -180;
	}
	return {
